    ...
    (async () => {
      try {
        ...
        if (userCartDocument && userCartDocument.items?.length) {
          ...
          const items: UserCartStreamItem[] = userCartDocument.items.map(
            (item: any) => mapUserCartResponse(item, STREAM_TYPE.BASE),
          );
...
const mapUserCartResponse = (
  ...
  const taxAmount =
    item.taxAmount !== undefined ? roundFloat(item.taxAmount) : undefined;
  const feePrice =
    item.feePrice !== undefined ? roundFloat(item.feePrice) : undefined;
